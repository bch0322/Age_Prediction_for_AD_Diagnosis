...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes=2, zero_init_residual=False,
        ...
        self.conv1 = nn.Conv3d(1, self.inplanes, kernel_size=5, stride=2, padding=2, bias=False)
        self.bn1 = norm_layer(self.inplanes)
        self.act_f = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool3d(kernel_size=3, stride=2, padding=1)

        self.age_encode = nn.Sequential(
            nn.Linear(1, 128),
            nn.ReLU(inplace=True),
            nn.Linear(128, self.inplanes),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, x_0, *args):
        ...
        x = self.bn1(x)
        x = self.act_f(x)
        x = self.maxpool(x)  # batch, 16, 31, 37, 31

        labels_age = args[0]  # (B,)
        x_age = self.age_encode(labels_age.unsqueeze(1))
        x_age = x_age.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand_as(x)
        x = x + x_age
        ...
        x_1 = self.fc(x)
        out_1 = x_1.view(x_1.size(0), -1)
        out_1 = torch.sigmoid(out_1).squeeze(-1)

        x_age = self.revgrad(x_age)
        x_2 = self.fc_age(x_age)
        out_2 = x_2.view(x_2.size(0))

        dict_result = {
            "logits": out_1,  # batch, 1
            "Aux_logits": [out_2],  # batch, 2
